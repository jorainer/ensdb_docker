...
createEnsDbForSpecies <- function(ftp_folder,
    ...
    for (i in 1:nrow(res)) {
        ...
        processOneSpecies(ftp_folder = paste0(ftp_folder, res[i, "folder"]),
                          ...
                          dropDb = dropDb,
                          dropIntermediateFiles = dropIntermediateFiles)
...
#'     EnsDb has been created.
#'
#' @param dropIntermediateFiles `logical(1)` whether the intermediate tab
#'     delimited txt files should be removed (defaults to `TRUE`).
#'
#' @author Johannes Rainer
#'
#' @noRd
